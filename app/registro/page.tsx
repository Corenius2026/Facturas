...
import { Receipt, User, Mail, Lock, ArrowRight, Loader2, AlertCircle, Inbox } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleGoogleRegister = async () => {
    ...
    try {
      ...
      if (error) {
        setErrorMessage('No pudimos iniciar registro con Google. Inténtalo nuevamente.');
    ...
    try {
      ...
      if (error) {
        if (error.message.toLowerCase().includes('already registered')) {
          setErrorMessage('Este correo ya se encuentra registrado. Intenta iniciar sesión.');
        } else {
          setErrorMessage('No se pudo completar el registro. Intenta con otra dirección de correo.');
        }
        setIsLoading(false);
        return;
      }
